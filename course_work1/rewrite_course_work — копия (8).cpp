// i dont use <iomanip> <ctime>, because i hope its no need
#include <iostream>
#include <string>
#include <fstream>
using namespace std; 
...
void sorting_by_age(data a[N], data a1_rew[N], data a2_rew[N], int &temp, temp_ b[N]){
...
void the_youngest_girl_by_age(data a[N], data a1_rew[N], data a2_rew[N], int &temp, int &count){
	...
	cout << a2_rew[count].name_of_student << " " << a2_rew[count].faculty_num << " " << a2_rew[count].av_balls << " " << a2_rew[count].age << " " << a2_rew[count].gender << endl;
	count = 0;
...
int main(){
	data a[N]; int choose = 0; data a1_rew[N]; data a2_rew[N]; bool d = 1; int count = 0; float sum = 0; int temp;
	...
	do{
		...
		if (!int(choose))
			...
			case 4:
				clear();
				sorting_by_age(a, a1_rew, a2_rew, temp, b);
