...
void debug(info arr[N]){
	...
	fstream f("test.txt", ios::out); fstream f2("random_names.txt", ios::out | ios::in | ios::app);
	...
	for(int i = 0; i < N; ++i) { // 0
		...
		if (arr[i].name_of_student == "")
			arr[formula].name_of_student = temp_names[i]; // 5 <- 100 2 = 1
		else 
			cout << arr[i].name_of_student << endl;
		//if (arr[i].name_of_student == arr[i + 1].name_of_student)
			//continue;
		 // if 2 == 1
			//cout << "same" << endl;
	}
	...
	//for(int j = 0; j < N; ++j)
	//for(int i = 0; i < N; ++i)
	//	if (arr[j].name_of_student == arr[i + 1].name_of_student)
	//		cout << "same" << endl;
