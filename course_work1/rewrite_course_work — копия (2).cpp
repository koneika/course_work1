...
string NAME = "test.txt";	// так же задаём переменную NAME для работы с файлами, 
...
void output(data a[N]){
	for(int i = 0; i < N ; ++i)
		cout	<< a[i].name_of_student << " " 
				<< a[i].faculty_num << " " 
				<< a[i].av_balls << " " 
				<< a[i].age << " " 
				<< a[i].gender << endl; // выводит имеющуюся информацию о студентах на экран: 5 имен студентов 5 факультет номеров, 
...
void input_data(data a[N]){	// функция/блок кода для ввода с клавиатуры данные 
	...
	char b; 
	f.open(NAME, ios::app); // открываем example.txt на чтен........ чтобы в дальнейшем не только отображать имеющуюся информацию, но и
							// ???
	...
	for(int i = 0; i < N; ++i) {
		...
		f >> a[i].name_of_student;								// после записывания с клавиатуры, оно так же записывает в файл
	...
	}															// так же само как я уже и писал, проверка является ли числом или дробным числом
