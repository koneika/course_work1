...
void owerwriting_the_data_for_man_and_woman(data a[N], data a1_rew[N], data a2_rew[N], int &count, double &sum){
...
void sorting_by_age(data a[N], data a1_rew[N], data a2_rew[N], int &temp, string &temp1, double &temp2){
	f.open(NAME, ios::out | ios::in | ios::app);	
	info_from_file_about_students(a);
	...
	for(int j = 0; j < N-1; ++j)
		for(int i = 0; i < N-1; ++i)
				if (a[i].age < a[i + 1].age) {
					...
					string temp1 = a[i].name_of_student;
...
void the_youngest_girl_by_age(data a[N], data a1_rew[N], data a2_rew[N], int &count , int &temp, string &temp1, double &temp2){
	f.open(NAME, ios::out | ios::in | ios::app);	
	info_from_file_about_students(a);
	...
	do{
		...
		if(a[N-1-i].gender == "Female"){
			cout << a[N-1-i].name_of_student << " " << a[N-1-i].faculty_num << " " << a[N-1-i].av_balls << " " << a[N-1-i].age << " " << a[N-1-i].gender << endl;	
			close++;
		}
		else if(N-1-i < 1){
...
int main(){
	srand(time(NULL));
	int choose = 0, count = 0, temp; double sum = 0, temp2; string temp1;
	...
	do{
		...
		if (!int(choose))
			...
			case 4:
				clear();
				sorting_by_age(a, a1_rew, a2_rew, temp, temp1, temp2);
			...
			case 5:
				...
				the_youngest_girl_by_age(a, a1_rew, a2_rew, count, temp, temp1, temp2);
