...
void output_from_file_info_about_students(data a[N]){	
		...
		f >> a[4].name_of_student >> a[4].faculty_num >> a[4].av_balls >> a[4].age >> a[4].gender;  // я не понимаю почему через for(int i...)
}

void important_output(data a[N]){	
		for(int i = 0; i < N ; ++i)
...
void f_youngest_and_sorting(data a[N], data a1_rew[N], data a2_rew[N], int &count, float &sum, int &temp/*, temp_male[N], temp_female[N]*/){
	...
	cout << "this is " << a[N-1].name_of_student << " minimal age is " << a[N-1].age << " she is " << a[N-1].gender << endl;
...
void youngest_girl(data a[N], data a1_rew[N], data a2_rew[N], int &temp){
system("cls");
	/*
	for(int i = 0; i < N; ++i) {
...
int main(){
	...
	do{
		...
		if (!int(choose))
			...
			case 4:
				clear();
				youngest_girl(a, a1_rew, a2_rew, temp);
