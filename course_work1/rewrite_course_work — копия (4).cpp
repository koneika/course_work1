...
//		a[i].gender = "Female";
//
//		f << a[i].name_of_student << " ";clear_output(a);
//		f << a[i].faculty_num << " ";
//		f << a[i].av_balls << " ";
//		f << a[i].age << " ";
//		f << a[i].gender << " " << endl;
...
void output(data a[N]){
	/*for(int i = 0; i < N ; ++i) {*/
		//f.open(NAME);
		//f >> a[i].name_of_student; // теперь 100% нашёл баг
		//f >> a[i].faculty_num;
		//f >> a[i].av_balls;
		//f >> a[i].age;
		//f >> a[i].gender;
		...
		f >> a[4].name_of_student >> a[4].faculty_num >> a[4].av_balls >> a[4].age >> a[4].gender;  // я не понимаю почему через for(int i...)
				...
				// выводит имеющуюся информацию о студентах на экран: 5 имен студентов 5 факультет номеров,// не работает правильно как надо
